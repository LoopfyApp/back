                    ...
                    <div class="card mb-4">
                        ...
                        <div class="card-body">    
                            ...
                            <div class="jumbotron">
                               ...
                               @include('admin.species.partials.modal_form')
                            </div>
        ...
        $.ajax({
                ...
                success:function(data){
                    ...
                    if(data.status==false){
                        alert('success', "{{__('species.notification.message')}}", "{{__('species.notification.save')}}");
                    }
                    else{
                        alert('error', "{{__('species.notification.message')}}","{{__('species.notification.error')}}")
                    }
  ...
  $('#table_species').on('click', '#editar_species', function () { 
                 ...
                 confirmation("{{__('general.information')}}", "{{__('general.edit')}}", 'warning', "{{__('general.edit')}}").then(datos=>{
                     if(datos.isConfirmed){
                        ...
                        $.ajax({
                            type:'GET',
                            url:"{{url('admin/species/info/')}}/"+Info,
...
function activar(idvalor){
    ...
    $.ajax({
                            type:'GET',
                            url:"{{url('admin/species/status/')}}/"+idvalor,

                            contentType: false,
                            processData: false,
                            success:function(data){
                                alert('success', "{{__('species.notification.message')}}", data.mensaje);
