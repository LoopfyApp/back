...
@section('title') {{__('services.title_consulta')}} @endsection
            ...
            <div class="col-12">
                ...
                <div class="card mb-4">
                    ...
                    <div class="card-body">
                        ...
                        <a type="button" href="{{route('business.veterinary.add')}}" style="width:35%" class="float-sm-right btn btn-primary btn-sm top-right-button "><i class="simple-icon-plus"></i>{{__('services.add_consulta')}}</a>
...
<script>
    ...
    var table = $('#table_services').DataTable({
        processing: true,
        serverSide: true,
        ajax: '{{route("business.veterinary.index")}}',
        columns: [{
                data: 'DT_RowIndex',
                name: 'DT_RowIndex'
            },
    ...
    function activar(idvalor) {
        ...
        $.ajax({
            type: 'GET',
            url: "{{url('business/veterinary/status/')}}/" + idvalor,
  ...
  $('#table_services').on('click', '#editar_species', function () { 
                 ...
                 confirmation("{{__('general.information')}}", "{{__('general.edit')}}", 'warning', "{{__('general.edit')}}").then(datos=>{
                     if(datos.isConfirmed){
                         window.location.href="{{url('business/veterinary/edit')}}/"+Info
